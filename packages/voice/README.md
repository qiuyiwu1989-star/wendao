...
### 3. 延迟的真凶：**第一句太长** + 提示词太长（实测把 5.9s 压到 3.0s）

先量后改。一次真实链路的分段实测（服务器侧中位数）：

| 环节 | 优化前 | 优化后 | 怎么改的 |
|---|---|---|---|
| VAD 等静音 | 900ms | 650ms | 阈值调紧 |
| ASR | 765ms | 854ms | 降采样 16kHz（本机看不出，移动网省更多） |
| **LLM 到可朗读** | **3526ms** | **860ms** | ↓ 见下 |
| TTS 首声 | 693ms | 665ms | — |
| **合计** | **5884ms** | **3029ms** | **-49%** |

LLM 那 2.7 秒是这么省的，两条独立的杠杆：

**① 让第一句变短（最大单项）。** 原来首句 43 字——首字 1.5s 就出来了，
却要等整句凑完才开口，**白等 2 秒**。在提示词里加一条"第一句不超过 12 字，
先接住再展开"，首句 43 字→8 字，实测 2147ms→883ms。

**② 首块允许在逗号处切**（`takeSentences(..., {firstChunkFast:true})`）。
只对第一块这么做，后面仍按整句，听感自然。

**③ 提示词砍到只留"影响对话行为"的部分**。语音场景用不上的（联网研究流程、
长篇输出模板、方法明细表…）纯拖累 TTFT：实测 1.8 万字→2.5 千字，
首字 1460ms→438ms。**砍"不适用"的，别砍"有用"的**——砍完必须回归测行为。
完整系统提示词几万字时，MiMo 冷缓存要重算，**首句能到 10-15 秒**。三件套缺一不可：
另外两条必备：**关掉思考**（`thinking:{type:"disabled"}`）、**句级流式朗读**（模组已内置）。
